...
// Configure connection strings on the Web App directly (avoid Service Linker dependency)
resource webConnectionStrings 'Microsoft.Web/sites/config@2022-09-01' = {
  name: 'connectionstrings'
  parent: web
  properties: {
    MyDbConnection: {
      value: 'Server=tcp:${dbserver.name}.database.windows.net,1433;Initial Catalog=${dbserver::db.name};Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;Authentication=Active Directory Managed Identity'
      type: 'SQLAzure'
      slotSetting: false
    }
  }
...
// Redis connection can be added later via Key Vault or app settings if needed.
...
output CONNECTION_SETTINGS array = [ 'MyDbConnection' ]
...
// Grant the Web App managed identity access to read Key Vault secrets
resource webKeyVaultSecretUserRoleAssignment 'Microsoft.Authorization/roleAssignments@2020-08-01-preview' = {
  scope: keyVault
  name: guid(resourceGroup().id, web.id, keyVaultSecretUserRoleRoleDefinition.id)
  properties: {
    roleDefinitionId: keyVaultSecretUserRoleRoleDefinition.id
